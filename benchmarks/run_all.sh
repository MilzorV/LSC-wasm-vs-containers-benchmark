...
if [[ "$PILOT" == "1" ]]; then
  ...
  python3 "$ROOT_DIR/benchmarks/run_load.py" --duration 3 --concurrency 2 --queries space,empty,enhanced --repeats 1
  ...
  python3 "$ROOT_DIR/benchmarks/run_load.py" --repeats 3
  python3 "$ROOT_DIR/benchmarks/run_load_hey.py" || true
