...
\begin{frame}{What we built}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, fill=DeckTeal!10, draw=DeckTeal, minimum width=4.8cm] (core) at (0,1.4) {\textbf{movie-search-core}\\fixture loading, tokenization, ranking, responses};
...
benchmarks/compare_results.sh
    ...
    \begin{column}{0.5\linewidth}
      ...
      \begin{block}{Acceptance rule}
        Both runtimes must return the same engine identity, document count, total hits, and ordered \texttt{hit.id} values.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.48\linewidth}
      \begin{table}
        ...
        \begin{tabular}{llrr}
          ...
          \midrule
          OCI & \texttt{/health} & 21.0 ms & 28.4 ms \\
          Spin & \texttt{/health} & 141.4 ms & 144.2 ms \\
...
\begin{frame}{Movie-search: throughput and tail latency}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\linewidth}
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{load_throughput.png}
    \end{column}
    \begin{column}{0.5\linewidth}
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{load_latency_p95.png}
    \end{column}
  \end{columns}
  ...
  \begin{tabularx}{\linewidth}{>{\bfseries}lX}
    ...
    \texttt{space} query & Spin had higher c=10 throughput, 77 req/s vs 54 req/s, but worse tails at high concurrency. \\
  \end{tabularx}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.58\linewidth}
      \begin{itemize}
        ...
        \item ``Wasm is lighter than containers'' is too simple: workload shape changed the result.
      \end{itemize}
    \end{column}
